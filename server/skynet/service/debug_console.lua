...
local function do_lua_cmd(session, address, cmd, ...)
	local func = LUA_COMMAND[cmd]
	if not func then
		error("no handle for " .. cmd)
	end
...
skynet.start(function()
	...
	skynet.register(".debug_console")
end)

local shutdown_cb = nil
function LUA_COMMAND.register_shutdown(session, address, cb_name)
	if shutdown_cb then
		error("can only have one shutdown callback")
	end
	shutdown_cb = {address, cb_name}
	skynet.retpack(true)
end

function LUA_COMMAND.lc_do_shutdown()
	skynet.retpack(COMMAND.shutdown())
end

local gm_cb = nil
function LUA_COMMAND.register_gm(session, address, cb_name)
	gm_cb = {address, cb_name}
	skynet.retpack(true)
end
...
function COMMAND.profile(...)
	local args_list = {...}
	local sec = math.ceil(tonumber(args_list[1]))
	local start_args = {}
	start_args.out_file_name = args_list[2]
	start_args.luaV_execute_begin = tonumber(args_list[3])
	start_args.luaV_execute_size = tonumber(args_list[4])
	for i=5, #args_list do
		start_args[1+#start_args] = tonumber(args_list[i])
	end
	
	local luaprofiler = require("luaprofiler")
	luaprofiler.start(start_args)

	for i=1,sec do
		skynet.sleep(100)
		print(string.format("profile... %d/%d", i, sec))
	end
...
function COMMAND.shutdown()
	local ret
	if not shutdown_cb then
		ret = "shutdown over"
	else
		ret = skynet.call(shutdown_cb[1], "lua", shutdown_cb[2])
	end
	pcall(function()
		skynet.call('.clusterd', "lua", "lc_shutdown")
	end)
	pcall(function()
...
function COMMAND.gm(uuid, name, ...)
	if not gm_cb then
		return "no gm"
	end
	return skynet.call(gm_cb[1], "lua", gm_cb[2], uuid, name, ...)
end
