...
function httpc.post_with_header(host, url, header, content_or_form, recvheader)
	local content
	if type(content_or_form) == "string" then
		content = escape(content_or_form)
	else
		local body = {}
		for k,v in pairs(content_or_form) do
			table.insert(body, string.format("%s=%s",escape(k),escape(v)))
		end
		content = table.concat(body , "&")
	end
	return httpc.request("POST", host, url, recvheader, header, content)
end
