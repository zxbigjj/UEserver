...
local busy_coroutine_pool = setmetatable({}, { __mode = "k" })
local coroutine_stop_cb_dict = setmetatable({}, { __mode = "k" })

local debug_info_dict = {
	free_coroutine = function() return #coroutine_pool end,
	busy_coroutine = function()
		local busy_count = 0
		for k, v in pairs(busy_coroutine_pool) do
...
function skynet.get_all_busy_coroutine()
	return busy_coroutine_pool
end

function skynet.set_coroutine_stop_cb(cb, co)
	co = co or coroutine.running()
	if not coroutine_stop_cb_dict[co] then
		pcall(profile.start)
	end
	coroutine_stop_cb_dict[co] = function()
		local used_time = profile.stop()
		cb(used_time)
	end
end

function skynet.info()
	local ret = {}
	for tag, func in pairs(debug_info_dict) do
		ret[tag] = func()
	end
	return ret
end

function skynet.report_debug_info(tag, func)
	debug_info_dict[tag] = func
end
	...
	if co == nil then
		co = coroutine_create(function(...)
			...
			while true do
				...
				busy_coroutine_pool[co] = nil
				local stop_cb = coroutine_stop_cb_dict[co]
				if stop_cb then
					coroutine_stop_cb_dict[co] = nil
					pcall(stop_cb)
				end
...
function skynet.clearco(co)
	local param = sleep_session[co]
	sleep_session[co] = nil
	if param then
		session_id_coroutine[param] = nil
		cleared_session_id[param] = true
	end
end
...
skynet.gettimeofday = c.gettimeofday
...
local function raw_dispatch_message(prototype, msg, sz, session, source)
	...
	if prototype == 1 then
		...
		if co == "BREAK" then
			session_id_coroutine[session] = nil
		elseif co == nil then
			if cleared_session_id[session] then
				cleared_session_id[session] = nil
			else
