...
cmd_stat(struct skynet_context * context, const char * param) {
	if (strcmp(param, "name") == 0) {
		sprintf(context->result, "%s", context->mod->name);
	}
	else if (strcmp(param, "mqlen") == 0) {
...
static const char *
cmd_stat_service(struct skynet_context * context, const char * param) {
	int size = strlen(param);
	char tail[size+1];
	uint32_t handle = 0;
	if (sscanf(param,"%d %s",&handle,tail) < 2){
		return NULL;
	}
	struct skynet_context * ctx = skynet_handle_grab(handle);
	if (ctx == NULL)
		return NULL;
	const char * ret = cmd_stat(ctx, tail);
	skynet_context_release(ctx);
	return ret;
}
